...
func TestServerSupervisesCancellationAndBoundedQueue(t *testing.T) {
	...
	if err := server.enqueueJob(second.ID); err != nil {
		t.Fatal(err)
	}
	if err := server.enqueueJob(third.ID); !errors.Is(err, ErrJobQueueFull) {
		t.Fatalf("third enqueue error = %v, want ErrJobQueueFull", err)
	}

	started := time.Now()
	if err := server.requestCancellation(first.ID); err != nil {
		t.Fatal(err)
	}
...
func TestCancelledJobCanBeDeleted(t *testing.T) {
	...
	if err := server.jobManager.DeleteJob(job.ID); err != nil {
		t.Fatal(err)
	}
