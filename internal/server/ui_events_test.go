...
func TestJobLifecyclePublishesUIEventsInOrder(t *testing.T) {
	...
	if err := manager.MarkJobCompleted(job.ID); err != nil {
...
func TestJobMetricsEndpointReturnsBoundedTail(t *testing.T) {
	...
	for iteration := 1; iteration <= 3; iteration++ {
		if err := server.jobManager.RecordMetrics(job.ID, MetricSample{Iteration: iteration, Cost: float64(iteration)}); err != nil {
